...
export async function POST(req) {
  ...
  items.forEach((item, i) => {
    ...
    page.drawRectangle({ x: ML, y: y - rowH, width: A4W-ML-MR, height: rowH,
      borderColor: rgb(0.74, 0.84, 0.93), borderWidth: 0.3 });
  ...
  page.drawText(totalStr, {
    x: cols[5].x + cols[5].w - fontB.widthOfTextAtSize(totalStr, 7) - 2,
    y: y - rowH + 6, size: 7, font: fontB, color: white
  });

  // Footer
  y -= rowH + 10;
  page.drawText("Exchange Rates: All amounts in SGD – no conversion required for this voucher.",
    { x: ML, y, size: 6.5, font: fontR, color: gray });
  y -= 10;
  page.drawText(`Amount in Words: ${amountInWords(total)}`,
    { x: ML, y, size: 7.5, font: fontB, color: black });
  y -= 14;
  page.drawText("Payment Mode:   Cash  /  Cheque No. ____________________________",
    { x: ML, y, size: 8, font: fontR, color: black });
  y -= 28;
